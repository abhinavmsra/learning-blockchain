...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            const blockchainErrors = await self.validateChain();

            if(blockchainErrors.length) {
                return reject('Invalid Blockchain, no more blocks can be added!!!'); 
            }
            ...
            block.time = self._currentTimestamp();
    ...
    requestMessageOwnershipVerification(address) {
        ...
        return new Promise((resolve) => {
            resolve(`${address}:${self._currentTimestamp()}:starRegistry`);
        });
    }
        ...
        return new Promise(async (resolve, reject) => {
            self.chain.forEach(async (block) => {
                ...
                if(block.previousBlockHash !== previousBlockHash) {
                    errorLog.push({ block, error: 'Blockchain Forked' }); 
                }
            });
